...
export function compression3_three(ns, decompressed){
    ...
    for (var i = 0; i < decompressed.length; i++) {
      ...
      let chunk = "";
      if (chunk_type == 2) {
        chunk = decompressed.slice(i, i + 2);
        i++;
        output = process_chunk(ns, output, chunk, chunk_type);
        chunk_type = 1;
      } else {
        chunk = decompressed.slice(i + 1, i + parseInt(decompressed[i]) + 1);
        i += parseInt(decompressed[i]);
        output = process_chunk(ns, output, chunk, chunk_type);
        chunk_type = 2;
      }
    }
